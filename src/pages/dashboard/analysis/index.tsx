...
import { Column, measureTextWidth, Pie } from '@ant-design/plots';
...
const Analysis = () => {
  ...
  function renderStatistic(containerWidth: number, text: string, style: unknown) {
  ...
  const config1 = {
    ...
    label: {
      ...
      autoRotate: false,
      content: ({ type, percent }: { type: any; percent: any }) => `${type}\n${(percent * 100).toFixed(0)}%`.toString(),
    } as any,
    statistic: {
      title: {
        offsetY: -4,
        customHtml: (container: any, _: any, datum: any) => {
      ...
      content: {
        ...
        customHtml: (container: any, _: any, datum: any, data: any) => {
          const { width } = container.getBoundingClientRect();
          const text = datum ? `¥ ${datum.value}` : `¥ ${data.reduce((r: any, d: any) => r + d.value, 0)}`;
          return renderStatistic(width, text.toString(), {
            fontSize: 32,
          });
        },
      },
